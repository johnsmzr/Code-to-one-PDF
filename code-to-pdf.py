...
code_extensions = [
    ...
    ".h", ".hpp", ".cs", ".rb", ".pl", ".sh", ".sql", ".json",
    ".xml", ".yaml", ".yml", ".md", ".rst", ".txt"
]
    ...
    for root, dirs, files in os.walk(source_folder, topdown=True):
        ...
        for filename in files:
            if any(filename.endswith(ext) for ext in code_extensions):
                if filename not in excluded_files and not any(fnmatch.fnmatch(filename, pattern) for pattern in excluded_patterns):
                    if not any(root.startswith(os.path.join(source_folder, d)) for d in excluded_dirs):
